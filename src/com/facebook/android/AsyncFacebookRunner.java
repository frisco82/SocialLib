...
public class AsyncFacebookRunner {
        ...
        new Thread() {
            @Override public void run() {
                try {
                    ...
                    if (response.getCode() != 200 || response.getContents().length() == 0 || response.getContents().equals("false")){
                        listener.onFacebookError(new FacebookError(
                                "auth.expireSession failed"), state);
                        return;
                    }
                    listener.onComplete(response.getContents(), state);
    ...
    public void request(final String graphPath,
        ...
        new Thread() {
            @Override public void run() {
                try {
                    HttpResponseWrapper resp = fb.request(graphPath, parameters, httpMethod);
                    listener.onComplete(resp.getContents(), state);
                } catch (FileNotFoundException e) {
                    listener.onFileNotFoundException(e, state);
                } catch (MalformedURLException e) {
                    listener.onMalformedURLException(e, state);
